  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "0 10px 16px", flex: "1 1 auto" }}>
        ...
        {/* Current folder row, so the tree also shows "you are here" alongside ancestors. */}
        {folder ? (
          <FolderRow
            folder={folder}
            isEditing={editingId === `ancestor:${folder.id}`}
            editValue={editingValue}
            onStartEdit={() => startEdit(`ancestor:${folder.id}`, folder.name)}
            onEditChange={setEditingValue}
            ...
            <div key={sf.id} style={{ marginBottom: "4px" }}>
              ...
              {expanded ? (
                <div style={{ paddingLeft: "28px" }}>
                  {ledgers.map((doc) => {
                    const isSelected = selectedLedgerId === doc.id;
                    return (
                      <div
                        key={doc.id}
