...
//
// `userOrgs` is fetched server-side by app/templates/new/page.js (same
// listUserOrgs() call DealList.jsx/KanbanDashboard.jsx use for their own
// org pickers) and passed in as a prop -- this component only decides
// whether to SHOW a picker (more than one org) or just default silently
// (exactly one), matching that existing pattern. Without this, every
// template created here fell through to the caller's PERSONAL org
// (POST /api/templates's getPersonalOrgId fallback), so members of the
// same business org could never see each other's templates (I1,
// final-review.md).
...
export default function NewTemplateForm({ userOrgs = [] }) {
  ...
  async function handleSaveTemplate(finalAnchors) {
    ...
    const res = await fetch("/api/templates", {
      ...
      body: JSON.stringify({
        ...
        fields,
        // Omitted entirely when there's nothing to pick (0 or 1 org) --
        // POST /api/templates falls back to getPersonalOrgId in that case,
        // same as DealList/KanbanDashboard's "New Deal" flow.
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 28px" }}>
      ...
      {needsOrgPick && stage !== "ready" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
          <p style={{ fontSize: "0.85rem", color: "var(--text-secondary)" }}>Which organization is this template for?</p>
