...
export async function POST(request) {
  try {
    ...
    const buffer = await buildLOIPdf(model, { watermark: forceWatermark });

    return new NextResponse(buffer, {
      status: 200,
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${forceWatermark ? "Letter_of_Intent_SAMPLE.pdf" : "Letter_of_Intent.pdf"}"`,
      },
    });
  } catch (err) {
