...
export const ROLES_BY_DOCUMENT_TYPE = {
  ...
  commercial_lease: ["landlord", "listing_agent", "buyers_agent", "transaction_admin", "attorney", "other"],
...
export function isValidRole(documentType, role, roleOtherLabel) {
  // custom_template has no fixed role enum -- its signer roles come from
  // that specific template's own TemplateAnchor.role values (e.g.
  // "Agency", "Buyer", "Signature"), which vary per uploaded template.
  // Any non-empty string is accepted; getRoleLabel already falls back to
  // the raw role string for anything not in ROLE_LABELS, so no roleOtherLabel
