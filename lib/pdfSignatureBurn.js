...
import { findBestNameForLine, groupAnchorsBySlot, percentBoxToPoints } from "./signatureAnchors.js";
...
export async function burnSignatures(pdfBuffer, signedSlots, signatureAnchors) {
  ...
  // Explicit-anchor pass: whatever the sender placed/confirmed in the
  // signature-placement review step (see SignatureAnchorReview.jsx) takes
  // priority over the auto-detected line/trailing-page fallback below.
  // Any slot with no explicit anchor is untouched here and falls through
  // to exactly today's behavior.
  const anchorsBySlot = groupAnchorsBySlot(signedSlots, signatureAnchors);
  if (anchorsBySlot.size > 0) {
    const pages = doc.getPages();
    const stillUnplaced = [];
    for (const slot of unplacedSlots) {
      const slotAnchors = anchorsBySlot.get(slot.id);
      if (!slotAnchors || slotAnchors.length === 0) {
        stillUnplaced.push(slot);
        continue;
      }
      for (const anchor of slotAnchors) {
        ...
        const { x, y, width, height } = percentBoxToPoints(anchor, pageWidth, pageHeight);

        if (anchor.type === "date") {
          page.drawText(`Signed: ${slot.signedAt}`, { x, y: y + height / 2 - 4, size: 8, font, color: rgb(0.4, 0.4, 0.4) });
          continue;
        }
        // "signature" and "initials" both draw the same captured
        // signatureImageDataUrl -- there is no separate initials-capture
        // step in app/api/sign/[token]/route.js today (see the design
        // spec's "Out of scope" note), so an "initials" anchor is simply
        // a smaller placement of the same signature image.
        if (slot.signatureImageDataUrl && slot.signatureImageDataUrl.startsWith("data:image/png")) {
          try {
            ...
            page.drawImage(pngImage, { x: x + (width - w) / 2, y: y + (height - h) / 2, width: w, height: h });
          } catch (err) {
            page.drawText("[signature image could not be rendered]", { x, y, size: 8, font, color: rgb(0.5, 0.5, 0.5) });
          }
        }
        // Mirror the fallback pass below: always stamp the execution date
        // near the signature image, not only when a sender explicitly
        // placed a separate "date" anchor. Without this, every anchor-placed
        // signature would burn with no visible signed date, unlike the
        // fallback line-matching and trailing-page paths which both draw
        // this unconditionally.
        page.drawText(`Signed: ${slot.signedAt}`, { x, y: Math.max(y - 10, 20), size: 7, font, color: rgb(0.4, 0.4, 0.4) });
      }
    }
    unplacedSlots = stillUnplaced;
  }
