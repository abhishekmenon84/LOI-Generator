...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  // Scopes the session cookie to the whole ledgerlot.ca domain (leading
  // dot) rather than the default host-only scope -- required now that the
  // app lives on app.ledgerlot.ca while some request paths (or a future
  // "logged in as X" marketing-header check) may touch the root domain.
  // Only overrides `domain`; every other cookie option (name, httpOnly,
  // sameSite, etc.) stays whatever Auth.js already defaults to. Omitted
  // entirely outside production -- there's no real subdomain split in
  // local dev/preview deployments (see middleware.js), and a `.ledgerlot.ca`
  // domain cookie would just never match those hosts anyway.
  cookies: process.env.NODE_ENV === "production" ? { sessionToken: { options: { domain: ".ledgerlot.ca" } } } : undefined,
  session: {
