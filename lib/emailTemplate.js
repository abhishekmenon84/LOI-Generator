...
// Ledgerlot's own mark (dark rounded square + white document glyph), a
// small static PNG at public/email-logo.png. Two earlier approaches both
// failed in real inboxes and are deliberately NOT used:
//  - inline <svg>: Gmail drops it outright, leaving just the dark square
//    with no glyph.
//  - a `data:image/png;base64,...` URI: renders in some clients but Gmail
//    (the client that matters most here) frequently shows a broken-image
//    icon for it instead -- data URIs are a well-known Gmail image
//    blind spot, unlike this app's earlier assumption. A real hosted
//    <img src> URL is the one approach every major client (Gmail, Outlook,
//    Apple Mail) reliably renders, which is why email logos are normally
//    hosted rather than inlined.
// Falls back to the old data URI only when NEXT_PUBLIC_APP_URL isn't
// configured (e.g. some local/dev setups) -- there'd be no absolute URL to
// point at otherwise. Regenerate public/email-logo.png if this mark ever
// changes; it must stay pixel-matched to the SVG icon in
// components/SiteHeader.jsx.
const LEDGERLOT_LOGO_URL = process.env.NEXT_PUBLIC_APP_URL ? `${process.env.NEXT_PUBLIC_APP_URL}/email-logo.png` : null;
const LOGO_PNG_BASE64_FALLBACK =
...
export function renderEmail({ title, body, ctaLabel, ctaUrl, footerNote, brandName, brandLogoUrl }) {
  ...
  const logoBlock = brandLogoUrl
    ...
    : LEDGERLOT_LOGO_URL
      ? `<img src="${LEDGERLOT_LOGO_URL}" width="36" height="36" alt="${name}" style="width: 36px; height: 36px; border-radius: 9px; display: block;" />`
      : `<img src="data:image/png;base64,${LOGO_PNG_BASE64_FALLBACK}" width="36" height="36" alt="${name}" style="width: 36px; height: 36px; border-radius: 9px; display: block;" />`;
