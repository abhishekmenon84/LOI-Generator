...
// This form's own numbering convention is "SECTION N — TITLE" (not "N.
// TITLE" like Purchase LOI/Commercial Lease) -- see the formatNumber
// argument below. Covers Sections 1-7 of the standard form; "ADDITIONAL
// NOTES" is a separate, unnumbered, purely data-driven block (only shows
// when model.conditions is non-empty) and isn't part of this customizable
// set. This document type is tied to a real NB statutory form (Form 6);
// renaming/hiding a section here is the user's own choice to make (see
// this feature's design discussion) -- Ledgerlot doesn't restrict it, but
// hiding a required section does not remove any actual legal obligation,
// only its visible text in this rendered copy.
export const RESIDENTIAL_LEASE_SECTION_DEFS = [
  { key: "parties", defaultTitle: "PARTIES" },
  { key: "premises", defaultTitle: "PREMISES" },
  { key: "tenancy", defaultTitle: "LENGTH OF TENANCY" },
  { key: "rent", defaultTitle: "RENT" },
  { key: "deposit", defaultTitle: "SECURITY DEPOSIT" },
  { key: "assignment", defaultTitle: "ASSIGNMENT" },
  { key: "signatures", defaultTitle: "SIGNATURES" },
];

export const DEFAULT_RESIDENTIAL_LEASE_DATA = {
  currentDate: "",
  // Editable top-level document title (was fixed as "Residential Lease").
  documentTitle: "Residential Lease",
  // Per-section { title?, enabled? } overrides -- see
  // RESIDENTIAL_LEASE_SECTION_DEFS above.
  sectionOverrides: {},
...
export function buildResidentialLeaseModel(data) {
  ...
  const { headings, enabled } = resolveSectionHeadings(
    RESIDENTIAL_LEASE_SECTION_DEFS,
  ...
  return {
    ...
    documentTitle: (data.documentTitle || "").trim() || "Residential Lease",
    landlordName,
