...
import { sendEmail, EMAIL_FROM } from "./sendEmail";
...
export async function finalizeSignatureRequest(requestId) {
  ...
  // Prefer the frozen snapshot name over the live Ledger.name -- a sender
  // can still rename the Ledger before the request is locked, and every
  // webhook/email here should describe what signers actually signed, not
  // a later rename. Falls back to the live name for pre-migration rows
  // created before snapshotName existed (snapshotName is nullable).
  const snapshotLedgerName = sigRequest.snapshotName || sigRequest.ledger.name;
  ...
  const finalPdf = await burnSignatures(pdfBuffer, signedSlots, sigRequest.signatureAnchors);
  ...
  //
  // Resend does NOT throw/reject on a send failure (invalid recipient,
  // unverified sending domain, etc) -- it resolves with { error } in the
  // body, so a bare try/catch around resend.emails.send() never actually
  // caught this. sendEmail() (lib/sendEmail.js) normalizes that into a
  // real { ok, error } result.
  const results = await Promise.all(
    allRecipients.map((s) =>
      sendEmail(resend, {
        from: EMAIL_FROM,
        to: s.email,
        subject: `Fully signed: ${snapshotLedgerName}`,
        html: renderEmail({
          title: "Fully signed",
          body: `<strong>${escapeHtml(snapshotLedgerName)}</strong> has been signed by all parties. Your signed copy and Certificate of Completion are attached to this email.`,
        ...
        attachments: [
          { filename: `${snapshotLedgerName.replace(/[^a-z0-9]+/gi, "_")}_signed.pdf`, content: finalPdf.toString("base64") },
          ...(certificatePdf ? [{ filename: `${snapshotLedgerName.replace(/[^a-z0-9]+/gi, "_")}_certificate.pdf`, content: certificatePdf.toString("base64") }] : []),
        ],
      })
    )
  );
  ...
  if (failed.length > 0) {
    const message = `${failed.length}/${results.length} completion email(s) failed: ${failed[0].error}`;
    console.error("[signatureFinalize] completion email delivery failed:", message);
    Sentry.captureMessage(message, { tags: { signatureRequestId: sigRequest.id } });
    await prisma.signatureRequest.update({
      where: { id: sigRequest.id },
      data: { deliveryError: message.slice(0, 500) },
    });
  }
