...
export async function loadAccessibleFolder(folderId, userId) {
  ...
  if (membership) {
    if (membership.role === "admin" && !folder.restrictedToParticipants) {
    ...
    ? await prisma.folder.findMany({
        where: {
          AND: [
            ...
            {
              OR: [
                // Admins see every folder in their org EXCEPT ones a
                // creator has explicitly restricted to participants only
                // (see Folder.restrictedToParticipants) -- an admin who
                // also happens to be the creator still sees it via the
                // createdByUserId clause below.
                ...(adminOrgIds.length > 0 ? [{ orgId: { in: adminOrgIds }, restrictedToParticipants: false }] : []),
                ...(adminOrgIds.length > 0 ? [{ orgId: { in: adminOrgIds }, createdByUserId: userId }] : []),
