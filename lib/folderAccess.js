...
async function purgeExpiredTrash() {
  ...
  // Never purge a folder that still has children — cascading delete of an
  // active child tree on a 30-day timer is a silent data-loss trap; require
  // the children to be unnested or independently purged first.
  const purgeableIds = candidates.filter((f) => f.children.length === 0).map((f) => f.id);
  // "purge" here means "stop returning it from queries," never
  // prisma.folder.delete — see Global Constraints. There is nothing to do
  // here beyond what listAccessibleFolders's lifecycleFilter already does;
  // this function exists only as the equivalent hook point to
  // lib/orgAccess.js's purgeExpiredTrash for a future genuine-deletion pass.
}
...
export async function listAccessibleFolders(userId, options = {}) {
  ...
  const participantFolders = participations
    .filter((p) => includeArchived || includeTrashed || (!p.folder.archivedAt && !p.folder.deletedAt))
