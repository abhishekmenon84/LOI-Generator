...
// Ordered, fixed list of this document type's numbered sections -- see
// lib/sectionHeadings.js for how title/enabled overrides are resolved.
// Order itself is not user-editable (not requested), only each section's
// title text and on/off state.
export const LOI_SECTION_DEFS = [
  { key: "assets", defaultTitle: "ASSETS TO BE ACQUIRED AND TRANSACTION SCOPE" },
  { key: "price", defaultTitle: "PURCHASE PRICE AND ALLOCATION" },
  { key: "commission", defaultTitle: "BROKERAGE COMMISSION ARRANGEMENTS" },
  { key: "confidentiality", defaultTitle: "CONFIDENTIALITY" },
  { key: "conditions", defaultTitle: "CONDITIONS PRECEDENT AND STRATEGIC PROTECTIONS" },
...
export const DEFAULT_FORM_DATA = {
  currentDate: "",
  // Editable top-level document title, distinct from the numbered section
  // headings below -- e.g. lets a business/lease-context user rename this
  // away from "Letter of Intent to Purchase" without touching anything
  // else in the document.
  documentTitle: "Letter of Intent to Purchase",
  // Per-section { title?, enabled? } overrides -- see
  // lib/sectionHeadings.js's resolveSectionHeadings and LOI_SECTION_DEFS
  // above for the fixed ordered key list this maps into. Absent/undefined
  // for any key means "default title, enabled" -- an empty {} here (the
  // default) renders identically to every section hardcoded on, matching
  // this document type's behavior before this feature existed.
  sectionOverrides: {},
  closingDate: "",
...
export function buildLOIModel(data) {
  ...
  // `source`/`index` are additive (every existing consumer of `.label`/
  // `.value`/`.total` -- lib/pdfBuilder.jsx, lib/docxBuilder.js -- is
  // unaffected) tags the preview canvas uses to know which rows map 1:1 to
  // a raw `data.properties[index]` entry (editable in place) versus a
  // computed row (business/contents value, or the grand total itself --
  // never a single source field, so never directly editable).
  ...
  if (hasRE) {
    properties.forEach((p, index) => {
      ...
      grandTotal += val;
      allocationRows.push({ label: p.address || "Property", value: val, source: "property", index });
    });
  }
  if (hasOps) {
    ...
    if (bVal > 0) allocationRows.push({ label: `Strategic ${bizType} Business Operation Value`, value: bVal, source: "computed" });
    if (cVal > 0) allocationRows.push({ label: "Active Interior Assets, Contents, and Fixtures", value: cVal, source: "computed" });
  }
  allocationRows.push({ label: "AGGREGATE COMMITTED TRANSACTION TOTAL", value: grandTotal, total: true, source: "computed" });
  ...
  const { headings, enabled } = resolveSectionHeadings(LOI_SECTION_DEFS, data.sectionOverrides);
  // Section 7 (Agency Disclosure) was already conditionally rendered based
  // on whether any agency text was actually entered -- composed here with
  // the user's own enabled/disabled choice so hiding it manually still
  // works even if agencyDisclosures is non-empty, and it never shows for
  // an empty agencyDisclosures regardless of the toggle.
  enabled.agency = enabled.agency && agencyDisclosures.length > 0;

  return {
    date: data.currentDate || "",
    documentTitle: (data.documentTitle || "").trim() || "Letter of Intent to Purchase",
