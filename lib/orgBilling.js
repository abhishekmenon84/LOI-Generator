...
// Personal-tier equivalent of createOrgSubscriptionCheckout. Kept as a
// separate function (not a shared parameterized one) because the two
// checkout flows key off different tier tables (PERSONAL_TIERS vs.
// BUSINESS_SEAT_TIERS) with different metadata shapes, and forcing them
// through one function would need a branch inside anyway -- this reads
// clearer as two small functions than one with a type-switch.
export async function createPersonalSubscriptionCheckout({ org, tier, successUrl, cancelUrl }) {
  const stripe = getStripeClient();
    ...
    line_items: [
      {
        price_data: {
          ...
          recurring: { interval: "month" },
          unit_amount: tier.priceCents,
          product_data: { name: `Ledgerlot — ${tier.key === "personal_premium" ? "Personal Premium" : "Personal Premium Plus"}` },
        },
        quantity: 1,
      },
    ],
    metadata: { orgId: org.id, tierKey: tier.key, tierType: "personal" },
    success_url: successUrl,
    cancel_url: cancelUrl,
  });
