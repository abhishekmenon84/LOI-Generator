...
// A solo admin (1 seat, no teammates invited yet) is billed at the
// Growth rate -- the spec's brackets start at 2 seats, but a lone admin
// must still be able to subscribe rather than being blocked until they
// invite someone. Returns null only for a seat count of 0 (shouldn't
// happen -- an org always has at least its creating admin).
...
// Monthly document quota for a given seat count, per the spec's formula
// (seats x docsPerSeatPerDay x 30 days). Returns 0 if the seat count
// doesn't match any bracket (e.g. a lone-admin org still on trial).
...
export async function createOrgSubscriptionCheckout({ org, tier, successUrl, cancelUrl, seatCount }) {
  const stripe = getStripeClient();
